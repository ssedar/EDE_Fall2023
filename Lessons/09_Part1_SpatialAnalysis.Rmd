...
#looks like an oval... 1 means one deg lat and long... buffering in spherical coordinates so wont look like a circle on a plane (screen is a plane) --> ISSUE HERE IS WE WANT IT TO LOOK LIKE A CIRCLE ON A PLANE --> FIX with CRS
```
...
EPAair.sf.utm <- st_transform(EPAair.sf,32617)#transforms coordinates to use new CRS
...
#Plot the data -- axis still written in N/W but lines have a bend/distortion because its projected ... will zoom out in class... for each epa site calculate mean distance to the others... is there a relationship between distance from sites to plants and air quality??
ggplot() +
...
#CHALLENGE: Use ggplot to map EPA sites with different size based on average distance --> USE SCALE_SIZE_CONTINUOUS

EPAair.sf.utm$AvgDistToGasPlant <- mean_distances


ggplot() +
  geom_sf(data = EPAair.sf.utm, aes(size = mean_distances), color = 'blue') +
  geom_sf(data = GasPlants.utm, color = 'red', shape = 'O') +
  scale_size_continuous(range = c(2, 10)) +  # Adjust the range for point sizes
  labs(size = "Average Distance to Gas Plants")
