...
## 2.1 Read tabular data and convert to spatial features -- reading DATA from CSV
...
#Reveal the columns
colnames(huc8_sf)
#"FID"        "HUC_8"      "SUBBASIN"   "ACRES"      "SQ_MILES"   "HU_8_STATE" "DWQ_Basin"  "geometry"  
...
#CRS ID: ESPG 4326 - Coordinate system: Ellipsoidal 2D CS. Axes: latitude, longitude. Orientations: north, east. UoM: degree

#different CRS... counties used ID--4269, while HUC uses ESPG ID-4326 --- WHAT DOES THE User input mean?
...
mapview(huc8_sf, 
        zcol = "SUBBASIN", 
        col.regions = c("Upper Neuse" = "purple", "Other" = "orange"))



mapview(huc8_sf, col.regions = "orange")+mapview(huc8_sf_UN, col.regions ='purple', layer.name = 'Upper Neuse')
#layer.name ---> legend 
...
#Join the SVI attributes to the county spatial features by merging
...
mapview(counties_sf_join, 
  ...
  #COLORED BY PPL IN POVERTY
  #COLOR RAMP RED TO BLUE USING BREWER# CEX IS SIZING BY PARTICULATE MATTER
...
#HOW DO YOU DO THIS WHEN THERE ISNT A HUC8 COLUMN
...
wind_raw <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
  colClasses = c('HUC_12' = 'factor')) %>%  
  mutate(HUC_8 = substr(HUC_12,1,8)) %>% #no existing HUC8 column-- substring command.. specify string grabbing from the first to the eighth value
  group_by(HUC_8) %>% 
...
#Aggregate the data using group_by and summarize, just as you would a non-spatial dataframe --gloms counties together sumary calculates all land.. similar to regular df
...
Recall that we have two basic categories of coordinate systems: **geographic**, where coordinates are measured in angles (e.g. degrees of latitude a9nd longitude); and **projected**, where coordinates are measured in linear units (e.g. meters or feet). The former maps our features on a sphere, and the latter maps our features on a plane. And the process of migrating spherical coordinates to planar ones is call "projecting" - and it involves a lot of math as well as various assumptions about the shape of the Earth, which is not a perfect sphere.
...
On top of projecting any geographic data to adjust for the limitations of the software, you'll also be wise to ensure all use the same CRS. This can be done with the `st_transform` command, supplying the EPSG code of the CRS that you want your data to be in. Let's get our main five sf objects all into a consistent CRS. #26917 is distortion designed for north carolina
...
neuse_intersect_1 <- counties_sf_utm[neuse_sf,]
  ...
  neuse_intersect_2 <- counties_sf_utm %>% 
    filter(st_intersects(x = ., y = neuse_sf, sparse = FALSE)) #x = .(whatever you put here)

#View the result
myMap + mapview(neuse_intersect_2, alpha.regions = 0)

#Actually intersect the counties (not just select those that intersect--- chopping things off)
neuse_counties_sf <- neuse_sf %>% #cookie cutter
  st_intersection(counties_sf_utm) #cookie dough
