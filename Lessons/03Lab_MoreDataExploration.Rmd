...
vector1[3] #assess third element of vector one: 5.3 

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) #this makes a matrix from scratch...providing matrix a sequence of numbers 1-20 
matrix1
...
matrix1[4, ] #before the comma: row, after the comma: column
matrix1[ , 3] #
matrix1[c(12, 14)] #when specifying elements, 
matrix1[c(12:14)] #
matrix1[2:4, 1:3] #I want rows 2,3,4 and columns 1,2,3
...
```{r DataExercise-1a}
nc_temp_high <-c(82, 83, 84, 79, 71, 78, 82, 85, 84)
nc_temp_low <-c(56, 58, 60, 60, 62, 62, 64, 64, 62)
...
```{r DataExercise-1b}
nc_temp_high_C <- c((nc_temp_high - 32)*(5/9))
nc_temp_low_C <- c((nc_temp_low - 32)*(5/9))
...
df_nc_temp <- data.frame("HighTempF"=nc_temp_high, "HighTempC"=nc_temp_high_C, "LowTempF"=nc_temp_low, "LowTempC"=nc_temp_low_C)
df_nc_temp
...
# SD doesn't work with dataframes, but will work with a matrix, since all of a matrix will be in one class
