...
output: pdf_document
toc: yes
title: "SamanthaSedar_A06_CraftingReports.Rmd"
---
...
```{r project.setup, project.setup, echo=TRUE, results='hide', message=FALSE, warning=FALSE, error=FALSE}
#Import libraries
library(tidyverse);library(lubridate);library(here);library(knitr)

epa_data <- 
  read.csv("~/EDE_Fall2023/Data/Processed/Processed_KEY/EPAair_O3_PM25_NC1819_Processed.csv", stringsAsFactors = TRUE) %>% 
  mutate(Date = ymd(Date))

#Import EPA data (from the processed_KEY folder) & fix dates
#not finding epa_data <- read.csv(here("Data","Processed","processed_KEY","EPAair_O3_PM25_NC1819_Processed.csv"),stringsAsFactors = TRUE) %>% mutate(Date = ymd(Date))#
```
...
```{r data.summary, echo=FALSE, fig.cap="Mean Particulates (2.5mm)\n\n", message=FALSE, output=FALSE}
...
knitr::kable(summary_PM25, col.names = c("County", "µg/m3"), caption = "Mean Particulates (2.5mm)")

knitr::kable(summary_Ozone, col.names = c("County", "µg/m3"), caption = "Mean Ozone")
...
```{r, echo=FALSE, fig.cap="Monthly Ozone, 2018", fig.align="left", fig.height=7, fig.width=12}
ozone_2018 <- epa_data %>% 
  filter(year(Date) == 2018)

boxplot(Ozone ~ factor(month(Date), levels = 1:12, labels=month.abb), data = ozone_2018, xlab = "Month", ylab = "Concentration (ug/m^3)", main = "Recorded Ozone Levels by Month (2018)")


```

```{r, echo=FALSE, fig.align="left", fig.cap="Monthly Ozone, 2019", fig.height=7, fig.width=12}
ozone_2019 <- epa_data %>% 
  filter(year(Date) == 2019)
boxplot(Ozone ~ factor(month(Date), levels = 1:12, labels=month.abb), data = ozone_2019, xlab = "Month", ylab = "Concentration (ug/m^3)", main = "Recorded Ozone Levels by Month (2019)")
```
...
## Git Repository
https://github.com/ssedar/EDE_Fall2023
