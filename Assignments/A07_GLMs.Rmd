...
NTL_Processed_plot <- ggplot(NTL_Processed, 
  ...
  labs(title = "July Temperature by Depth", x = "Depth", 
       y = "Temperature (C)") 
...
> Answer: The AIC suggests that all of the explanatory variables are important and should be used to predict the temperature. Based on the r-squared, we can see that this model explains 74.12% of the variance, which is marginally higher than only using depth, which explains 73.87% of variance.
...
>Answer:Peter Lake's mean temperature is significantly different from all the other lakes. The confidence intervals do not contain zero and the p value is less than 0.05. Several lakes have statistically distinct mean temperatures from Central Long Lake and Crampton Lake. These lakes have p values less than 0.05 for comparisons with Central Long Lake and Crampton Lake, indicating significant differences.
