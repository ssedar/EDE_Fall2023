...
cheat sheets under help -- R and CRAN to search
...
# %in%-operator (not a pipe) + slice c(152:304)

```

```{r Exercise 1 & 2}
#filter subsets columns, select does rows
# Exercise 1: 
...
NTL.phys.data.1999 <- filter(NTL.phys.data, year4 %in% c(1990:1999) & lakename == "Tuesday Lake")
# c is a vector indicator
...
```{r Exercise 3 & 4}
#Example using pipes to wrangle data.. pipe into the next level command... if errors, run by segments between pipes
...
NTL.phys.data.processed.July <- 
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake")  %>%
  unique(NTL.phys.data$sampledate[format(NTL.phys.data$sampledate, "%m") == "07"]) 
